...
- **The date is rendered in ET.** Market data is ET-dated; 03:30 UTC is still
  the previous evening in New York, and a study labeled with tomorrow's date
...
Colors are `DefineGlobalColor` globals rather than inputs, because thinkScript
has no color input type; users recolor them under the study's **Globals**
section in Edit Studies.

Line **style and weight are deliberately not inputs.** thinkorswim already
gives every plot its own color/weight/style controls under Edit Studies, so a
