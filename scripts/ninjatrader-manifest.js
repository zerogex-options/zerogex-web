...
const hasZip = fs.existsSync(SOURCE_ZIP);
const zipHash = hasZip ? hashOf(SOURCE_ZIP) : null;
const zipPath = hasZip ? `/ninjatrader/ZeroGexGammaLevels.${zipHash}.zip` : null;
...
// Publish the hashed copies. The unhashed names are written by the Makefile
// (zip) or tracked in git (cs), so links already in the wild keep resolving.
...
if (hasZip) {
  ...
  console.log('  · no export archive — the pages will offer the .cs only');
}
