...
- **State Management**: React hooks with custom API data hooks
- **Real-time Updates**: Polling-based with configurable refresh intervals

### Key Features
- Real-time gamma exposure (GEX) dashboard
- Options flow analysis and smart money tracking
- Intraday trading tools (VWAP, ORB, volume spikes)
- Greeks and GEX analysis by strike
...
The application uses these environment variables (configured in `.env.local`):
...
```
Cannot find module '@/core/ThemeContext'
```
Solution: Verify ThemeContext.tsx exists in frontend/core/

**Import errors:**
```
'import', and 'export' cannot be used outside of module code
```
Solution: Move imports to top of file, outside of functions
...
# Optimized build with PM2 process management
```

## API Integration

The frontend is designed to work with a FastAPI backend. Update `.env.local` to point to your backend:
...
Available API endpoints (configured in frontend):
- `/api/gex/summary` - GEX summary data
- `/api/gex/by-strike` - GEX by strike
