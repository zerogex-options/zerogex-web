    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)]">Spread-ranking and sizing intelligence for translating directional context into executable options structures.</p>
        </div>
      ...
      {signal && (
        <section className="mb-8 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)] p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="rounded-lg bg-[var(--color-surface-subtle)] border border-[var(--color-border)] p-4">
              <div className="text-xs uppercase tracking-wide text-[var(--color-text-secondary)] mb-1 flex items-center gap-1"><CalendarClock size={14} /> Expiry / DTE</div>
          ...
          <div className="mt-4 rounded-lg bg-[var(--color-surface-subtle)] border border-[var(--color-border)] p-4">
            <div className="text-xs uppercase tracking-wide text-[var(--color-text-secondary)] mb-1">Strikes</div>
            <div className="font-mono text-base">{signal.top_strikes}</div>
          </div>
          <div className="mt-4 rounded-lg bg-[var(--color-surface-subtle)] border border-[var(--color-border)] p-4">
            <div className="text-xs uppercase tracking-wide text-[var(--color-text-secondary)] mb-2">Why the optimizer prefers it</div>
            <ul className="list-disc pl-5 space-y-2 text-sm leading-relaxed text-[var(--color-text-secondary)]">
      ...
      <section className="mb-8 bg-[var(--color-surface)] rounded-lg p-6 border border-[var(--color-border)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-80 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-3">
            ...
            <div className="h-80 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-3">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={candidateScoreChart} layout="vertical" margin={{ left: 24, right: 12, top: 8, bottom: 8 }}>
                  ...
                  <XAxis type="number" stroke="var(--color-chart-axis)" />
                  <YAxis type="category" dataKey="name" stroke="var(--color-chart-axis)" width={140} />
                  <Tooltip formatter={(value, name) => [value, name === 'edgeScore' ? 'Edge Score' : name === 'pop' ? 'POP' : 'EV']} />
                  <Bar dataKey="edgeScore" fill="var(--color-brand-primary)" radius={[0, 6, 6, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="space-y-3">
              {(signal.candidates || []).slice(0, 3).map((candidate) => (
                <div key={`${candidate.rank}-${candidate.strategy_type}-${candidate.strikes}`} className="rounded-lg border border-[var(--color-border)] p-4 bg-[var(--color-surface-subtle)]">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] p-3"><div className="text-xs text-[var(--color-text-secondary)] mb-1">POP</div><div>{(candidate.probability_of_profit * 100).toFixed(1)}%</div></div>
                    <div className="rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] p-3"><div className="text-xs text-[var(--color-text-secondary)] mb-1">Expected Value</div><div>{formatCompact(candidate.expected_value, { currency: true })}</div></div>
                    <div className="rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] p-3"><div className="text-xs text-[var(--color-text-secondary)] mb-1">Kelly Fraction</div><div>{(candidate.kelly_fraction * 100).toFixed(1)}%</div></div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="mb-8 bg-[var(--color-surface)] rounded-lg p-6 border border-[var(--color-border)]">
        ...
        <div className="text-sm text-[var(--color-text-secondary)] mb-4">Accuracy endpoint: <span className="font-mono">/api/signals/position-optimizer/accuracy</span></div>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
