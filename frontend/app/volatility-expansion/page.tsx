...
function interpretation(score: number | null, t: (key: string) => string) {
  if (score == null) return t('noReading');
  if (score >= 70) return t('bullishPrimed');
  if (score >= 30) return t('amplificationPotential');
  if (score <= -70) return t('bearishActive');
  if (score <= -30) return t('downsideAmplification');
  return t('neutralSuppressed');
}
  ...
  return (
    <PageShell>
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-6">
        ...
        <span className="text-sm italic text-[var(--color-text-secondary)]">{t('pageQuote')}</span>
        <TooltipWrapper
          text={t('tooltipText')}
          placement="bottom"
        >
      ...
      <section className="zg-feature-shell p-6">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2">
            <div className="text-xs uppercase tracking-[0.14em] text-[var(--color-text-secondary)] mb-2">{t('expansionScoreLabel')}</div>
            <AutoFitValue className="text-4xl sm:text-5xl md:text-6xl font-black leading-none" style={{ color }}>
              {score != null ? score.toFixed(2) : '—'}
            </AutoFitValue>
            <div className="mt-2 text-lg font-semibold">{interpretation(score, t)}</div>
            <p className="mt-4 text-sm text-[var(--color-text-secondary)]">
              {t('rangeSignFollows')} <code>direction_score</code>{t('magnitudeGatedBy')} <code>expansion</code>.
            </p>
            <ExpandableCard
              className="mt-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-3"
              expandTrigger="button"
              expandButtonLabel={t('expandScoreHistory')}
            >
              <div className="text-[11px] uppercase tracking-wider text-[var(--color-text-secondary)] mb-2">{t('scoreHistoryLabel')}</div>
          ...
          <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-5">
              <div className="flex items-center justify-between mb-1">
                <div className="text-sm font-semibold">{t('expansionLabel')}</div>
                <div className="text-xs text-[var(--color-text-secondary)]">{t('rangeZeroHundred')}</div>
              </div>
              ...
              <p className="mt-3 text-xs text-[var(--color-text-secondary)]">{t('gexReadinessDesc')}</p>
            </div>

            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-5">
              <div className="flex items-center justify-between mb-1">
                <div className="text-sm font-semibold">{t('directionLabel')}</div>
                <div className="text-xs text-[var(--color-text-secondary)]">{t('rangeNeg100Pos100')}</div>
              </div>
              ...
              <p className="mt-3 text-xs text-[var(--color-text-secondary)]">{t('momentumDesc')}</p>
            </div>

            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-5">
              <div className="text-sm font-semibold mb-1">{t('magnitudeLabel')}</div>
              <AutoFitValue className="text-2xl sm:text-3xl font-black">{magnitude != null ? magnitude.toFixed(1) : '—'}</AutoFitValue>
              <p className="mt-3 text-xs text-[var(--color-text-secondary)]">{t('magnitudeDesc')}</p>
            </div>

            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-5">
              <div className="text-sm font-semibold mb-1">{t('expectedMoveLabel')}</div>
              <AutoFitValue className="text-2xl sm:text-3xl font-black">{expectedBps != null ? `${expectedBps.toFixed(1)} bps` : '—'}</AutoFitValue>
              <p className="mt-3 text-xs text-[var(--color-text-secondary)]">{t('expectedMoveDesc')}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="zg-feature-shell mt-8 p-6">
        <h2 className="text-xl font-semibold mb-4">{t('contextBreakdownTitle')}</h2>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-4">
            <div className="text-sm font-semibold mb-2 flex items-center gap-2"><Info size={14} /> {t('howItsBuiltLabel')}</div>
            ...
            <div className="mt-3 pt-3 border-t border-[var(--color-border)] text-[11px] text-[var(--color-text-secondary)]">
              {t('signFlipPrefix')} <code>direction_score</code> {t('signFlipMiddle')} <code>expansion</code> {t('signFlipSuffix')}
            </div>
          </div>

          <div className="lg:col-span-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-subtle)] p-4">
            <div className="text-sm font-semibold mb-3">{t('liveInputsLabel')}</div>
            <div className="grid grid-cols-[minmax(150px,1.2fr)_1fr_minmax(120px,1fr)] gap-2 text-xs font-semibold uppercase tracking-wide text-[var(--color-text-secondary)] pb-2 border-b border-[var(--color-border)]">
              <span>{t('metricHeader')}</span>
              <span className="text-right">{t('valueHeader')}</span>
              ...
              <ContextRow label="Net GEX" value={formatGexCompact(ctx.net_gex)}>
                {ctx.net_gex != null ? (
                  <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${ctx.net_gex > 0 ? 'bg-[rgba(27,196,125,0.12)] text-[var(--color-bull)]' : 'bg-[rgba(255,77,90,0.12)] text-[var(--color-bear)]'}`}>
                    ...
                    {ctx.net_gex > 0 ? t('suppressionStatus') : t('amplificationStatus')}
                  </span>
              ...
              <ContextRow label="Momentum z" value={formatSigned(ctx.momentum_z, 3)}>
                {ctx.momentum_z != null ? (
                  <span className={`inline-flex items-center gap-1 text-xs font-medium ${ctx.momentum_z > 0.25 ? 'text-[var(--color-bull)]' : ctx.momentum_z < -0.25 ? 'text-[var(--color-bear)]' : 'text-[var(--color-text-secondary)]'}`}>
                    ...
                    {Math.abs(ctx.momentum_z) >= 1 ? t('strongStatus') : Math.abs(ctx.momentum_z) >= 0.25 ? t('moderateStatus') : t('flatStatus')}
                  </span>
                ) : <span className="text-xs text-[var(--color-text-secondary)]">—</span>}
              </ContextRow>
              <ContextRow label="5-bar % change" value={formatPct(ctx.pct_change_5bar, 3)}>
                {ctx.pct_change_5bar != null ? (
                  <span className={`inline-flex items-center gap-1 text-xs font-medium ${ctx.pct_change_5bar > 0.001 ? 'text-[var(--color-bull)]' : ctx.pct_change_5bar < -0.001 ? 'text-[var(--color-bear)]' : 'text-[var(--color-text-secondary)]'}`}>
                    ...
                    {Math.abs(ctx.pct_change_5bar) >= 0.005 ? t('strongStatus') : Math.abs(ctx.pct_change_5bar) >= 0.001 ? t('moderateStatus') : t('negligibleStatus')}
                  </span>
                ) : <span className="text-xs text-[var(--color-text-secondary)]">—</span>}
              </ContextRow>
            </div>
      ...
      <SignalEventsPanel signalName="vol_expansion" symbol={symbol} title={t('eventTimelineTitle')} />
    </PageShell>
  );
}
