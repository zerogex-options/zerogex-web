...
export default function MaxPainPage() {
  ...
  const { data: maxPainCurrent, loading: oiLoading, error: oiError } = useApiData<MaxPainCurrentResponse>(
    `/api/max-pain/current?symbol=${symbol}&strike_limit=80`,
    { refreshInterval: 30000 }
  );

  const { data: maxPainSeries, loading: seriesLoading, error: seriesError } = useApiData<MaxPainTimeRow[]>(
    `/api/max-pain/timeseries?symbol=${symbol}&timeframe=${timeframe}&limit=${getMaxDataPoints()}`,
  ...
  const underlyingNow = safeNum(quoteData?.close);

  const oiChart = [...(maxPainCurrent?.strikes || [])]
    .map((row) => ({ strike: safeNum(row.settlement_price), notionalOiM: safeNum(row.total_notional) / 1_000_000 }))
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <MetricCard title="Last Series Max Pain" value={latest?.maxPain ? `$${latest.maxPain.toFixed(2)}` : '--'} tooltip="Most recent max pain from /api/max-pain/timeseries." theme="dark" />
          <MetricCard title="Underlying Price" value={underlyingNow ? `$${underlyingNow.toFixed(2)}` : '--'} trend={underlyingNow && currentMaxPain ? (underlyingNow > currentMaxPain ? 'bullish' : 'bearish') : 'neutral'} tooltip="Current underlying quote from /api/market/quote." theme="dark" />
        </div>
      </section>

      <section className="mb-8 bg-[#423d3f] rounded-lg p-6">
        <SectionTitle title="Notional Open Interest by Strike" tooltip="Strike-level total notional from /api/max-pain/current." />
        {oiError ? <ErrorMessage message={`Error loading data: ${oiError}. Try a lower strike_limit or verify backend max-pain pipeline.`} /> : oiChart.length === 0 ? <div className="text-gray-400 text-center py-8">No max pain OI data available</div> : (
          <ResponsiveContainer width="100%" height={360}>
            <BarChart data={oiChart} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
              ...
              <YAxis stroke="#f2f2f2" tickFormatter={(value) => `${Number(value).toFixed(1)}M`} domain={['auto', 'auto']} />
      ...
      <section className="mb-8 bg-[#423d3f] rounded-lg p-6">
        <SectionTitle title="Max Pain vs Underlying Price" tooltip="Max pain timeseries with live underlying overlay; y-axis scales to data." />
