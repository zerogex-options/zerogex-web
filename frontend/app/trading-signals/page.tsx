...
import { ArrowDown, ArrowUp, Brain, ExternalLink, Info, ShieldCheck, TableProperties, TrendingUp, Zap } from 'lucide-react';
...
function ActionCardSection({ data, error, onRetry, hideLowConfidence, onToggleHide }: ActionCardSectionProps) {
  ...
  // STAND_DOWN cards aren't persisted, so they never carry an id. Trade cards
  // always do (the /action endpoint now attaches the row id returned by
  // insert_action_card), so the "View permalink" deep-link is safe to show
  // whenever data.id is present.
  const permalinkId = !isStandDown && typeof data?.id === 'number' ? data.id : null;
...
// (TradeCard, StandDownCard, PriceCell, LegRow, structureLabel, directionColor
// were extracted to components/ActionCard.tsx so the same renderer powers
// both this live dashboard and the shareable /cards/[id] permalink page.)
