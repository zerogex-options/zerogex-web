...
function isUnexpiredOptionExpiration(expiration: string, now = new Date()) {
  if (!expiration) return false;
  const parsed = new Date(expiration);
  if (Number.isNaN(parsed.getTime())) return false;

  const expirationEnd = new Date(parsed);
  expirationEnd.setHours(23, 59, 59, 999);
  return expirationEnd >= now;
}
...
export default function FlowAnalysisPage() {
  ...
  const expirationOptions = useMemo(
    () =>
      Array.from(new Set((flowByExpiration || []).map((r) => r.expiration).filter(Boolean)))
        .filter((expiration) => isUnexpiredOptionExpiration(expiration))
        .sort(),
    [flowByExpiration],
  );
  const [selectedExpirations, setSelectedExpirations] = useState<Set<string>>(new Set());
  const activeSelectedExpirations = useMemo(() => {
    const validExpirations = new Set(expirationOptions);
    return new Set(Array.from(selectedExpirations).filter((expiration) => validExpirations.has(expiration)));
  ...
  const expirationRowsFiltered = useMemo(() => {
    const source = (flowByExpiration || []).filter((r) => isUnexpiredOptionExpiration(r.expiration));
    if (activeSelectedExpirations.size === 0) return source;
    return source.filter((r) => activeSelectedExpirations.has(r.expiration));
  }, [flowByExpiration, activeSelectedExpirations]);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <section className="mb-8 bg-[#423d3f] rounded-lg p-6">
        ...
        <MultiSelectChips
          ...
          selected={activeSelectedExpirations}
