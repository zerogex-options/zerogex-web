...
export const metadata = articleMetadata('how-to-trade-around-gamma-flip');

const articlePath = path.join(process.cwd(), 'content/articles/how-to-trade-around-gamma-flip.md');

export default function HowToTradeAroundGammaFlipPage() {
  const markdown = fs.readFileSync(articlePath, 'utf8');

  return (
    <div className="mx-auto max-w-4xl px-6 py-12">
      <ArticleJsonLd slug="how-to-trade-around-gamma-flip" />
      ...
      <article className="rounded-3xl border border-[var(--color-border)] bg-[var(--bg-card)]/95 px-8 py-10 shadow-[0_20px_60px_var(--color-info-soft)] md:px-14">
        <div className="mb-8 text-sm uppercase tracking-[0.2em] text-[var(--text-muted)]">ZeroGEX Education • 10 min read</div>
        <div className="blog-medium-style">{renderMarkdown(markdown)}</div>
      </article>

      <RelatedArticles slug="how-to-trade-around-gamma-flip" />
      ...
      <div className="zg-feature-shell mt-8 p-6 md:p-8">
        ...
        <h3 className="mb-2 text-xl font-semibold text-[var(--color-text-primary)]">
          See today&apos;s gamma flip and regime live
        </h3>
        <p className="mb-5 text-sm leading-7 text-[var(--color-text-secondary)]">
          The ZeroGEX free dashboard surfaces the gamma flip with live distance from spot and the Net GEX magnitude — the regime read that decides which playbook is on right now.
        </p>
        <Link
