...
function renderMarkdown(markdown: string): ReactNode[] {
  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith('```')) {
      ...
      out.push(
        <pre key={`code-${i}`} className="my-6 overflow-x-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-subtle)] px-4 py-3 text-[13px] leading-6 text-[var(--text-primary)]">
          <code>{codeLines.join('\n')}</code>
        </pre>,
      );
      continue;
