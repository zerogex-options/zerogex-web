...
export const metadata = articleMetadata('why-we-dont-publish-dex');

const articlePath = path.join(process.cwd(), 'content/articles/why-we-dont-publish-dex.md');

export default async function WhyWeDontPublishDexPage() {
  const markdown = await loadLocalizedMarkdown(articlePath);

  return (
    <div className="mx-auto max-w-4xl px-6 py-12">
      <ArticleJsonLd slug="why-we-dont-publish-dex" />
      ...
      <article className="rounded-3xl border border-[var(--color-border)] bg-[var(--bg-card)]/95 px-8 py-10 shadow-[0_20px_60px_var(--color-info-soft)] md:px-14">
        <ArticleMeta slug="why-we-dont-publish-dex" />
