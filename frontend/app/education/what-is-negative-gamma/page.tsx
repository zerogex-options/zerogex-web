...
export const metadata = articleMetadata('what-is-negative-gamma');

const articlePath = path.join(process.cwd(), 'content/articles/what-is-negative-gamma.md');

export default function WhatIsNegativeGammaPage() {
  const markdown = fs.readFileSync(articlePath, 'utf8');

  return (
    <div className="mx-auto max-w-4xl px-6 py-12">
      <ArticleJsonLd slug="what-is-negative-gamma" />
      ...
      <RelatedArticles slug="what-is-negative-gamma" />
      ...
      <div className="zg-feature-shell mt-8 p-6 md:p-8">
        ...
        <h3 className="mb-2 text-xl font-semibold text-[var(--color-text-primary)]">
          See whether today is positive or negative gamma
        </h3>
        <p className="mb-5 text-sm leading-7 text-[var(--color-text-secondary)]">
          The ZeroGEX free dashboard surfaces Net GEX and the gamma flip for SPY, SPX, and QQQ — the two reads that tell you whether dealers are long gamma or short gamma right now.
        </p>
        <Link
