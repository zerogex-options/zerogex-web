...
export const metadata = articleMetadata('gamma-vwap-confluence-explained');

const articlePath = path.join(
  process.cwd(),
  'content/articles/gamma-vwap-confluence-explained.md',
);

export default async function GammaVwapConfluenceExplainedPage() {
  const markdown = await loadLocalizedMarkdown(articlePath);

  return (
    <div className="mx-auto max-w-4xl px-6 py-12">
      <ArticleJsonLd slug="gamma-vwap-confluence-explained" />
      ...
      <article className="rounded-3xl border border-[var(--color-border)] bg-[var(--bg-card)]/95 px-8 py-10 shadow-[0_20px_60px_var(--color-info-soft)] md:px-14">
        <ArticleMeta slug="gamma-vwap-confluence-explained" />
        <div className="blog-medium-style">{renderMarkdown(markdown)}</div>
      </article>

      <ArticleFaq slug="gamma-vwap-confluence-explained" />
