...
function GaugePanel({ data }: { data: MarketTideResponse }) {
  ...
  return <section className={`${panel} overflow-hidden`} aria-labelledby="tide-gauge-title">
    <h3 id="tide-gauge-title" className="zg-h3">Current reading</h3>
...
function ContributorTable({ title, rows, empty }: { title: string; rows: MarketTideComponent[]; empty: string }) {
  ...
  return <section className={panel}><h3 className="zg-h3">{title}</h3>
...
export default function MarketTidePage() {
  ...
  return <PageShell className="space-y-8">
    <header>
      <h1 className="text-3xl font-bold mb-2">Market Tide</h1>
      <p className="text-[var(--text-secondary)]">Market-wide options pressure adjusted for the dealer gamma regime.</p>
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <p className="text-sm text-[var(--text-muted)]">Last updated: {formatTimestamp(data?.timestamp)} {data && loading && <span className="ml-2 inline-flex items-center gap-1"><RefreshCw size={11} className="animate-spin motion-reduce:animate-none"/> Refreshing</span>}</p>
        <fieldset><legend className="mb-2 text-sm text-[var(--text-secondary)]">Window</legend><div className="inline-flex rounded-md border p-1" style={{ backgroundColor: "var(--bg-card)", borderColor: "var(--color-border)" }}>{WINDOWS.map(value=><button key={value} type="button" aria-pressed={windowMinutes===value} onClick={()=>setWindowMinutes(value)} className="rounded px-4 py-1.5 text-sm font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 aria-pressed:bg-[var(--color-brand-primary)] aria-pressed:text-white">{value}m</button>)}</div></fieldset>
      </div>
    </header>
    {!data && loading ? <MarketTideSkeleton/> : !data && error ? <ErrorMessage message={error} onRetry={refetch}/> : data && <>
      ...
      <section>
        <SectionHead title="Market Tide Snapshot" sub="Direction, gamma regime, and breadth across the supported symbol basket." />
        <GaugePanel data={data}/>
      </section>
      <div className="grid gap-4 md:grid-cols-3">
        <MetricCard title="Flow Direction" value={formatNumber(flow)} subtitle={flow == null ? "Unavailable" : Math.abs(flow) < .005 ? "Balanced" : flow > 0 ? "Call-led / bullish" : "Put-led / bearish"} trend={flow == null || Math.abs(flow) < .005 ? "neutral" : flow > 0 ? "bullish" : "bearish"} tooltip="Directional aggregate options pressure; it does not replace the Market Tide label." />
        <MetricCard title="Gamma Regime" value={formatNumber(gamma)} contextBadge={<span className="text-sm font-semibold">{gammaLabel}</span>} subtitle={gammaCopy} tooltip="Gamma changes how strongly directional flow may move the market; it is not itself bullish or bearish." />
        <section className={panel} style={participation < 60 ? { borderColor: "var(--color-bear)" } : undefined}><h3 className="zg-eyebrow">Participation</h3><div className="zg-metric mt-3 text-3xl">{formatNumber(data.eligible_symbols, 0)} <span className="text-lg text-[var(--text-secondary)]">of {formatNumber(data.configured_symbols, 0)}</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-[var(--color-border)]" role="progressbar" aria-label="Market Tide participation" aria-valuemin={0} aria-valuemax={100} aria-valuenow={participation}><div className="h-full bg-[var(--color-brand-primary)]" style={{width:`${participation}%`}}/></div><p className="mt-2 text-sm font-semibold">{formatNumber(data.participation_pct, 1)}% {participation < 60 && "· Below 60% minimum"}</p></section>
      </div>
      <section className={panel}><h3 className="zg-h3">Market Breadth</h3><div className="mt-5 flex h-5 overflow-hidden rounded-full" aria-label="Bullish, neutral, and bearish market breadth"><div className="bg-[var(--color-bull)]" style={{width:`${breadth[0]}%`}}/><div className="bg-[var(--color-border)]" style={{width:`${breadth[1]}%`}}/><div className="bg-[var(--color-bear)]" style={{width:`${breadth[2]}%`}}/></div><div className="mt-4 grid gap-2 text-sm sm:grid-cols-3"><span>▲ Bullish breadth <strong>{formatNumber(data.bullish_breadth_pct,1)}%</strong></span><span className="sm:text-center">● Neutral breadth <strong>{formatNumber(data.neutral_breadth_pct,1)}%</strong></span><span className="sm:text-right">▼ Bearish breadth <strong>{formatNumber(data.bearish_breadth_pct,1)}%</strong></span></div></section>
      <section><SectionHead title="What is moving the tide?" sub="Symbol-level contributions to the current market-wide reading." /><div className="grid gap-5 xl:grid-cols-2"><ContributorTable title="Leaders" rows={Array.isArray(data.leaders)?data.leaders:[]} empty="No positive contributors in this window."/><ContributorTable title="Laggards" rows={Array.isArray(data.laggards)?data.laggards:[]} empty="No negative contributors in this window."/></div></section>
      {Array.isArray(data.stale_symbols) && data.stale_symbols.length > 0 && <details className={panel} style={participation < 60 ? { borderColor: "var(--color-bear)" } : undefined}><summary className="cursor-pointer font-semibold">Stale or unavailable symbols ({data.stale_symbols.length})</summary><div className="mt-4 flex flex-wrap gap-2">{data.stale_symbols.map((symbol,i)=><span key={`${symbol}-${i}`} className="rounded-full border px-3 py-1 text-xs font-mono" style={{borderColor:"var(--color-border)"}}>{symbol || "Unknown"}</span>)}</div></details>}
    </>}
  </PageShell>;
}
