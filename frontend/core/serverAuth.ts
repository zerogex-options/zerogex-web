...
function getSessionByToken(token: string): SessionWithUser | null {
  ...
  return {
    ...
    user: {
      ...
      hasPriorPaid:
        row.paid_welcome_email_sent_at != null ||
        (row.subscription_lapsed as number | null) === 1,
...
function createSessionForUser(user: AuthUser) {
  ...
  // behaviour that actually matters after a credential compromise. What is gone
  ...
  if (!user) {
    ...
    try {
      ...
      if (emailRow) {
        ...
        // a bad/absent code leaves the signup organic.
