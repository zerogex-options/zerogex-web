...
Market-makers (the "dealers") are typically the counterparty when you trade an option — buy a call and a desk is usually short it. To stay flat directionally, they hedge by buying or selling the underlying. As the stock moves, the option's hedge ratio (delta) changes, so the dealer has to keep re-buying or re-selling. Crucially, *who* is long or short isn't visible in public data — so ZeroGEX **models** dealer positioning with the traditional call-positive / put-negative open-interest convention (dealers assumed net long calls, net short puts). It's a chosen convention, not observed inventory.

**Gamma** is how fast that hedging requirement changes. **GEX** ("gamma exposure") translates the whole option chain's gamma into dollars — roughly, *the dollar amount of underlying dealers would need to trade for every 1% move in the stock under that model.*

Under the model, two regimes are separated by a single price level called the **gamma flip**:

- **Above the flip — dealers modeled net long gamma.** They *tend* to sell as the stock rises and buy as it falls → mean-reverting / volatility-suppressing.
- **Below the flip — dealers modeled net short gamma.** They *tend* to buy as it rises and sell as it falls → momentum-amplifying / volatility-expanding.

Realized behavior still depends on actual flow, liquidity, vol, catalysts, and how well the dealer-positioning assumption holds.
...
4. Multiply each contract's gamma by `OI × 100 × S² × 0.01` (the industry "dollar GEX per 1% move" convention used by SpotGamma / SqueezeMetrics / Cheddar Flow), and apply the modeled dealer sign convention (calls +, puts − — the assumption that dealers are long calls and short puts, not an observed position).
...
| **Unusual Whales** | Per-strike GEX aggregation (cumulate gamma × OI by strike) | Cheap to compute; very fast; intuitive per-strike bar chart | Not the spot-shift definition — a cumulative-by-strike "zero gamma" level is a simplified approximation; freezes when the true zero-gamma is outside the ingested strike band |
