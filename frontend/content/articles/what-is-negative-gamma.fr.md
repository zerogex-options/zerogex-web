...
**Le gamma négatif**, dans le contexte de l'options-flow, décrit un régime : les dealers qui se trouvent de l'autre côté des transactions d'options des clients sont modélisés comme ayant un book net short-gamma. La conséquence pratique : quand SPY monte, ils ont tendance à *acheter* SPY pour rester couverts, et quand SPY baisse, ils ont tendance à *vendre* SPY. Leurs opérations de hedging vont **dans le même sens** que le prix — pas à contre-courant.

Ce réflexe mécanique transforme le book des dealers en amplificateur. Les selloffs ont tendance à s'accélérer. Les rallyes ont tendance à s'étendre. La volatilité intraday réalisée tend à être plus élevée que l'implicite. Le comportement de pin tend à se rompre. Le même setup graphique qui fonctionnait hier (quand les dealers étaient long gamma et absorbaient les mouvements) se fait écraser aujourd'hui (quand ils sont short gamma et courent après le mouvement).
...
Le gamma est un Greek d'options du second ordre qui mesure comment le delta d'une option évolue lorsque le sous-jacent bouge. Un chiffre de "gamma exposure" signé est le gamma agrégé sur l'ensemble du book *modélisé* des dealers. Le gamma brut est positif pour toute option *longue* — call ou put — de sorte que le signe doit provenir d'une hypothèse sur ce que détiennent les dealers : les calls modélisés comme détenus long (contribuant un gamma positif) et les puts modélisés comme détenus short (contribuant un gamma négatif). Le signe négatif du côté put est la position *short* modélisée, non le fait que les puts seraient « à gamma négatif » par nature.

Lorsque le *net* de ces contributions signées est négatif, le book est modélisé comme short gamma dans l'ensemble. La façon conventionnelle dont cela apparaît dans les outils de flow : Net GEX < 0.

> Il s'agit d'une convention modélisée, non d'un inventaire observé. Elle utilise l'hypothèse traditionnelle d'open interest call-positif / put-négatif ; le positionnement réel des dealers n'est pas directement observable à partir des données publiques de la chaîne d'options.

La convention standard — les clients vendent des calls en overwriting et achètent des puts pour se protéger — implique que les dealers sont typiquement longs de calls et short de puts, mais les *magnitudes* varient selon le positioning. Lorsque la demande des clients penche fortement vers les puts (par exemple pendant les régimes de peur), l'inventaire de puts short croissant du book des dealers peut pousser le gamma net en négatif ; lorsque les calls dominent (par exemple lors de tendances haussières calmes où l'overwriting construit du gamma long-call chez les dealers), le book est modélisé long gamma.
...
2. Pour rester delta-neutre, ils **achètent** généralement le sous-jacent pour compenser la baisse.
...
Si vous voulez voir le Net GEX du jour, le gamma flip et la lecture de régime en direct pour SPY, SPX, QQQ et NDX — les trois chiffres qui vous disent si les dealers sont actuellement long gamma ou short gamma — la vue gratuite gamma-levels de ZeroGEX les affiche tous.
