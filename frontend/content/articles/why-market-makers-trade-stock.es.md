...
*Cuando los market makers operan en acciones, a menudo no es porque tengan una view direccional. Es porque el delta de las opciones que mantienen no deja de cambiar — y a medida que cambia, por lo general necesitan ajustar la cobertura en el subyacente para mantenerse casi planos. Ese flujo de cobertura es una de las fuentes de order flow más estructuralmente estimables del mercado.*
...
Un market maker que os vende una call, por lo general, no está tratando de expresar una visión bajista. Quiere el spread — los pocos centavos entre el bid y el ask — y por lo general quiere mantener su exposición direccional cerca de neutral. Vender la call lo deja corto de delta, así que compra acciones frente a esa posición hasta que la posición está aproximadamente delta-neutral. Eso es el delta-hedging, y es una parte central del modelo económico de un dealer de opciones: almacenar la opción, neutralizar la dirección, cobrar el edge.

El problema es que "plano" no es un lugar al que se llega una sola vez. Es un lugar al que el dealer vuelve una y otra vez a lo largo de la sesión, porque el delta de un book de opciones rara vez se queda quieto. Y aquí está la parte que importa para quien lee el flow: cuando ese delta se mueve, la cobertura resultante suele estar guiada por la gestión de riesgo y no por una view direccional. Ninguna convicción, ningún pronóstico de mercado — el riesgo del book cambió, así que el dealer por lo general necesita ajustar. *Cuándo* y *cómo* lo hace queda a su discreción. La dirección probable y el tamaño aproximado de ese ajuste son las partes más estimables.

Esa distinción — guiado por el riesgo frente a discrecional — es la razón por la que el hedging de los dealers se puede leer. El flow discrecional es una suposición sobre lo que un trader *quiere* hacer. El flow de cobertura es una estimación de lo que un dealer probablemente *necesitará* hacer para mantenerse casi plano. Uno depende de la intención del trader. El otro está limitado por la mecánica de la cartera.
...
Así que la cobertura son en realidad dos tareas, no una. Primero el dealer neutraliza el nivel *actual* del delta del book — la operación única que deja la posición casi plana. Luego viene la tarea continua: a medida que se operan nuevas opciones y que el spot, el tiempo y la vol mueven ese delta, el dealer reajusta para mantenerse casi plano. La cobertura inicial se establece cuando se abre la posición. Lo que aparece en el tape es el flujo interminable de re-hedges que persiguen al delta mientras deriva. Entender qué mueve el delta es entender de dónde viene la presión de cobertura.
...
- **Precio spot.** Cuando la acción se mueve, el delta de cada opción se mueve con ella. La sensibilidad del delta al spot es la **gamma**. Este es el componente reactivo — responde cuando el precio se mueve, y su efecto puede ser grande e inmediato.
- **Tiempo.** A medida que se acerca el vencimiento, el delta deriva incluso con el spot inmóvil: las opciones out-of-the-money se deslizan hacia delta 0, las in-the-money se mueven hacia delta ±1 (+1 para las calls, −1 para las puts). La sensibilidad del delta al tiempo es el **charm**. Funciona de forma continua, ocurra algo o no.
...
Precio, reloj y miedo. Esas son las tres grandes palancas, y el dealer está expuesto a las tres. Cuando cualquiera de ellas se mueve, arrastra el delta del book fuera de su cobertura y genera presión para ajustar la cobertura en acciones. Estas no son las *únicas* variables — los tipos de interés, los dividendos, los cambios en la superficie de volatilidad, los supuestos de financiación y las nuevas operaciones de opciones que entran en el book también empujan el delta —, pero intradía suelen ser de segundo orden frente al spot, el tiempo y la vol. El efecto combinado es lo que llamamos **Forced Flow**: una estimación de las acciones que un dealer por lo general necesitará comprar o vender para mantenerse cubierto a medida que evolucionan el spot, el tiempo y la vol.
