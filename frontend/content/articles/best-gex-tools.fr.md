...
Un constat se situe sous les trois approches et mérite d'être lu avant chacune d'elles. La recherche publiée par FirmTape, confrontée aux données de transactions étiquetées par participant du Cboe pour une seule séance (2025-03-28), rapporte que sur l'ensemble du carnet SPX 0DTE à la mi-séance — 1,84 million de contrats — la variation de position nette de la capacité teneur de marché était de 0,10 % des contrats, tandis que les flux customer et pro-customer évoluaient à +2,24 % et -2,78 % l'un contre l'autre. Sur cette séance, le dealer a terminé quasiment à plat, et l'inventaire circulait entre types de clients plutôt que vers le carnet d'un dealer. Tout modèle qui divise le monde en clients et dealers — chaque outil de cet article, ZeroGEX compris — attribue ce flux inter-clients au dealer. C'est une seule séance, et le fournisseur qui l'a publiée est celui qu'elle dessert le plus, ce qui plaide pour lui plutôt que contre lui. Traitez cela comme la question ouverte qui sous-tend toute la catégorie et non comme un résultat acquis, et traitez le « carnet dealer » de n'importe quel fournisseur comme un modèle de positionnement, pas comme une mesure d'inventaire. C'est la norme que cet article applique aux niveaux de ZeroGEX eux-mêmes sur la [page méthodologie](/methodology), et elle vaut ici aussi.
...
FirmTape est l'autre outil de ce groupe qu'il faut connaître, et il repose sur d'autres fondations : le carnet dealer est reconstruit print par print à partir du tape d'options plutôt qu'à partir de l'open interest et d'une convention de signe, son flip de gamma zéro est publié avec une incertitude annoncée plutôt que comme un chiffre nu, et une archive de replay gratuite des séances SPX passées se trouve derrière. L'archive est la partie inhabituelle. FirmTape annonce environ 1 100 séances SPX terminées remontant à avril 2022, une ajoutée chaque soir, chacune rejouable seconde par seconde depuis l'ouverture, sans différé, sans troncature et sans compte — et sur la journée encore en cours, des niveaux gratuits différés de 15 minutes. L'accès intraday en direct coûte 49 $ par mois ; un produit de crédits de recherche et une licence de données sont facturés séparément et ne sont nécessaires ni pour les niveaux ni pour l'archive. FirmTape référence aussi un serveur hébergé dans le registre officiel du Model Context Protocol sous `com.firmtape/spx-options-gamma`, de sorte qu'un assistant peut lire directement les niveaux d'une séance. Si vous ne faites confiance à aucune convention de signe, c'est l'outil de la catégorie conçu pour vous — sous réserve de la mise en garde du critère 3 ci-dessus.

Sur cette mise en garde : FirmTape est le seul fournisseur de cet article à publier sa propre précision de signage, et le chiffre mérite d'être lu avant d'accepter « mesuré plutôt que supposé » comme une amélioration nette. Sa recherche rapporte que la règle de cotation standard signe correctement les prints SPXW 0DTE sur 52,3 % du volume lorsqu'elle est confrontée aux données Cboe étiquetées par participant pour une seule séance, qu'un correctif documenté porterait ce chiffre à 74,5 %, et que ce correctif n'était pas encore dans le pipeline de production en septembre 2026. Une étude distincte, également sur une seule séance, de son signeur de paquets multi-jambes rapporte 80,4 % de jambes client correctement signées, couvrant 40,2 % des prints SPX du jour mesuré. Les dénominateurs diffèrent — volume, jambes, prints — de sorte que ces chiffres ne se combinent pas en une précision unique du produit, et les deux études reposent sur une seule séance. En publier quoi que ce soit est assez rare pour être porté au crédit de l'outil. Mettre les 52,3 % en balance avec le discours reste le travail du lecteur.
...
- **Un carnet dealer signé sans précision de signage publiée.** Tout outil qui déduit l'inventaire dealer à partir du tape prend une décision d'achat ou de vente sur chaque print, et cette décision est mesurablement juste ou fausse. Si le fournisseur ne dit pas à quelle fréquence il a raison face à une source indépendante, l'inventaire est une affirmation et non une mesure. Quand un fournisseur publie bien un chiffre, vérifiez deux choses avant de le porter à son crédit : combien de séances il couvre, et si le pipeline évalué est celui qui tourne actuellement en production. Un correctif documenté mais non déployé n'améliore pas les niveaux que vous consultez aujourd'hui.
