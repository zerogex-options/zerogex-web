...
- **A free hosted MCP server** on those same delayed levels, at `https://zerogex.io/mcp`, so Claude, ChatGPT, Cursor or any other Model Context Protocol client can read the flip and the walls directly in a conversation. No key and no account; the real-time API stays a Pro feature.
