...
Un flip vide signifie autre chose lorsque le filtre **Échéance** ne retient qu’une partie de la chaîne. Le graphique trace alors les niveaux des échéances que vous avez choisies, et leur flip est reconstruit à partir de ces seuls strikes ; or un sous-ensemble est souvent d’un seul signe (un carnet 0DTE d’après-midi à gamma négatif sur tous les strikes ne franchit jamais le zéro), il n’y a donc aucun passage à tracer. La pastille le dit directement : `NO FLIP IN SELECTED EXPIRIES`. Contrairement au cas précédent, celui-là ne se résoudra *pas* sur un instantané ultérieur, car rien ne manque. Remettez **Échéance** sur **Toutes** pour voir le flip de la chaîne entière - le niveau que rapporte la page Dealer Positioning, qui lit la chaîne complète et continue donc d’afficher un nombre pendant que le graphique est restreint.
