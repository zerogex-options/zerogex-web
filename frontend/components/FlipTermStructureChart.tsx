    ...
    <ExpandableCard expandTrigger="button" expandButtonLabel="Expand chart" className="h-full">
      ...
      >
        <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs" style={{ color: textColor }}>
            <div className="flex items-center gap-1.5">
              <span className="inline-block h-0.5 w-4" style={{ backgroundColor: FLIP_LINE_COLOR }} />
              Today
            </div>
            <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-1.5">
              <span style={{ color: colors.bearish, fontWeight: 700, fontSize: 14, lineHeight: 1 }}>
                ×
              </span>
              Unresolved
            </div>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <div className="ml-2 inline-flex gap-1">
            ...
            <button
              type="button"
              onClick={() => applyPreset([5, 20, 60, 120])}
              className="px-2 py-1 text-xs rounded border"
              style={{
                backgroundColor: 'var(--color-surface-subtle)',
                borderColor: 'var(--color-border)',
        ...
        {error ? (
          <div className="flex items-center justify-center h-[300px] text-sm" style={{ color: colors.bearish }}>
            {error === 'No data available yet'
              ? `No usable option snapshot for ${symbol} — check ingestion.`
              : `Backend error: ${error}`}
          </div>
        ) : loading && !data ? (
          <div className="flex items-center justify-center h-[300px] text-sm" style={{ color: mutedText }}>
        ...
        {/* Caveat for historical dots — the persisted rows are production
            flips snapshotted h days ago, not h-day-horizon flips. */}
        {hasHistorical && (
          <p className="mt-2 text-[11px] leading-relaxed" style={{ color: mutedText }}>
            ...
            &ldquo;h-day-horizon flip h days ago.&rdquo; A diamond above today&apos;s spot means the regime
            sat above price h days ago; below means it sat below.
          </p>
        )}
