...
function AnalogClock({ time, label, theme }: { time: Date; label: string; theme: Theme }) {
  ...
  const digital = time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false });

  return (
    <div className="flex flex-col items-center gap-1">
      <svg width="70" height="70" viewBox="0 0 100 100">
        <title>{`${label}: ${digital}`}</title>
        <circle cx="50" cy="50" r="48" fill="none" stroke={colors.muted} strokeWidth="3" />
        {[...Array(12)].map((_, i) => {
...
export default function WorldClocks({ theme, compact = false }: WorldClocksProps) {
  const [times, setTimes] = useState({ ny: new Date(), london: new Date(), tokyo: new Date() });
