...
export default function GammaHeatmap() {
  ...
  const getColor = (value: number, maxAbsValue: number) => {
    ...
    const bearishTone = { r: 255, g: 90, b: 95 };
    const neutralTone = theme === 'dark'
      ? { r: 27, g: 49, b: 67 }
      : { r: 247, g: 245, b: 247 };
    const bullishTone = { r: 47, g: 191, b: 113 };
    ...
    <ExpandableCard expandTrigger="button" expandButtonLabel="Expand chart">
      <div className="rounded-lg overflow-hidden" style={{ backgroundColor: theme === 'dark' ? colors.cardDark : colors.cardLight, border: `1px solid ${colors.muted}` }}>
        ...
        >
          ...
          <g>
            <line x1={plotLeft + 290} x2={plotLeft + 310} y1={20} y2={20} stroke={colors.accent} strokeWidth={2.25} />
          ...
          {gammaFlipPath && (
            <g clipPath="url(#heatmapClip)">
              <path d={gammaFlipPath} fill="none" stroke={colors.accent} strokeWidth={2.25} />
              {hoveredIdx != null && gammaFlipPoints
                .filter((p) => p.idx === hoveredIdx)
                .map((p) => (
                  <circle key={`gamma-flip-dot-${p.ts}`} cx={p.x} cy={p.y} r={3.5} fill={colors.accent} />
                ))}
            </g>
          )}
