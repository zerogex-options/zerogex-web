...
help:
	...
	@echo "  make forecast-range-width [SYMBOL=SPX] [SESSIONS=120] [TARGET=0.8] - Read-only: by how much is the morning projected range wider than it needs to be? Finds the smallest per-side scale factor that would still have contained each graded session, and reports the percentile of those that hits the coverage target. In-sample — a starting estimate to watch forward, not a tuned constant. JSON=1 for machine-readable output"
...
forecast-range-width:
	@cd frontend && bash -lc 'source $$HOME/.nvm/nvm.sh && nvm use 22 >/dev/null && node --experimental-strip-types --no-warnings scripts/forecast-range-width.mts $(if $(SYMBOL),--symbol $(SYMBOL),) $(if $(SESSIONS),--limit $(SESSIONS),) $(if $(TARGET),--target $(TARGET),) $(if $(JSON),--json,)'
