.PHONY: help install dev build rebuild start stop restart logs status users referrals migrate-tiers all-to-pro delete-user seed-founders grant-founding clear-zombie-customers webhook-health trial-reminders public-cohort backup-monitoring backup-auth clean deploy logo blog-images
...
help:
	...
	@echo "  make public-cohort - Break the tier='public' cohort into reactivation segments (EMAILS=1 for paste-ready lists, COHORT=<name> to filter)"
...
# to one segment (e.g. COHORT=founding-eligible EMAILS=1).
public-cohort:
	@cd frontend && bash -lc 'source $$HOME/.nvm/nvm.sh && nvm use 22 >/dev/null && node --no-warnings scripts/list-public-cohort.mjs $(if $(EMAILS),--emails,) $(if $(COHORT),--cohort $(COHORT),)'
