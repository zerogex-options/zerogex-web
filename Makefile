...
help:
	...
	@echo "  make cancellation-alerts - Email yourself one alert per cancellation that SAID WHY, at the moment the member clicks Cancel and answers the survey. Churn carrying no reason at all is skipped, and so is the terminal 'lapsed' row (it repeats the same survey answer weeks later, after the save window has closed). Sweeps the audit log (so a failed send retries instead of vanishing) and latches each event once. Driven every 15m by the cancellation-alerts systemd timer. DRY_RUN=1 to preview, SINCE=<YYYY-MM-DD|iso> to backfill history, PREVIEW_TO=<email> for a sample, MARK_ONLY=1 to silence a backlog without emailing (ignores LIMIT — it takes the whole backlog), KIND=pending|lapsed|both (default pending), INCLUDE_SILENT=1 to also see churn that captured no reason, LIMIT=<n>/LOOKBACK=<hours>/THROTTLE_MS=<ms> to tune, TO=<email> to override the recipient"
	...
	@echo "  make forecast-range-width [SYMBOL=SPX] [SESSIONS=120] [TARGET=0.8] [MODEL=heuristic_v1_4] - Read-only: by how much is the morning projected range wider than it needs to be? Finds the smallest per-side scale factor that would still have contained each graded session, and reports the percentile of those that hits the coverage target. History spans several range-model generations and a pooled factor describes a blend that no longer runs, so MODEL=<name> restricts it to the live one. In-sample — a starting estimate to watch forward, not a tuned constant. JSON=1 for machine-readable output"
...
# Rebuild the per-day rollup behind Admin -> Monitoring -> Growth, then
...
# Repair for a campaign email that PROMISED the extended trial but did not claim
# users.reactivation_email_sent_at — the column app/api/billing/checkout reads
# before granting REACTIVATION_TRIAL_DAYS. Without the stamp /pricing shows the
# longer trial (it reads the number off ?reactivate=1) while Stripe still
# charges after the standard 7 days. Selects by the campaign's own audit rows
# (CAMPAIGN=<key>, default product_update_2026_08) or one account (EMAIL=<addr>),
# skips anyone with prior paid history (they get no trial either way) and anyone
# already stamped. Stamps with the time the offer was made, not now. Sending
# does this by itself as of send-product-update.mts's grantsExtendedTrial flag;
# this exists for the batch that went out before that. DRY_RUN=1 lists, YES=1
# applies.
backfill-reactivation-entitlement:
	@cd frontend && bash -lc 'source $$HOME/.nvm/nvm.sh && nvm use 22 >/dev/null && node --experimental-strip-types --no-warnings scripts/backfill-reactivation-entitlement.mts $(if $(CAMPAIGN),--campaign $(CAMPAIGN),) $(if $(EMAIL),--email $(EMAIL),) $(if $(DRY_RUN),--dry-run,) $(if $(YES),--yes,)'
...
forecast-range-width:
	@cd frontend && bash -lc 'source $$HOME/.nvm/nvm.sh && nvm use 22 >/dev/null && node --experimental-strip-types --no-warnings scripts/forecast-range-width.mts $(if $(SYMBOL),--symbol $(SYMBOL),) $(if $(SESSIONS),--limit $(SESSIONS),) $(if $(TARGET),--target $(TARGET),) $(if $(MODEL),--model $(MODEL),) $(if $(JSON),--json,)'
