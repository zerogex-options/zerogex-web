...
cancellation-alerts:
	@cd frontend && bash -lc 'source $$HOME/.nvm/nvm.sh && nvm use 22 >/dev/null && node --experimental-strip-types --no-warnings scripts/send-cancellation-alerts.mts $(if $(TO),--to $(TO),) $(if $(SINCE),--since $(SINCE),) $(if $(LOOKBACK),--lookback $(LOOKBACK),) $(if $(LIMIT),--limit $(LIMIT),) $(if $(KIND),--kind $(KIND),) $(if $(PREVIEW_TO),--preview $(PREVIEW_TO),) $(if $(MARK_ONLY),--mark-only,) $(if $(DRY_RUN),--dry-run,)'
